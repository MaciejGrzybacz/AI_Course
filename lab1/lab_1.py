...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# your_code
# in Iowa there is a similar temperature as in Poland
df['CentralAir'] = df['CentralAir'].apply(lambda x: x if pd.notnull(x) else 'N')
df['EnclosedPorch'] = df['EnclosedPorch'].apply(lambda x: x if pd.notnull(x) else 0)
df['Fireplaces'] = df['Fireplaces'].apply(lambda x: x if pd.notnull(x) else 0)
df['SaleCondition'] = df['SaleCondition'].apply(lambda x: x if pd.notnull(x) else 'Normal')
...
lasso_cv = LassoCV(alphas=None, n_alphas=1000, random_state=0)
